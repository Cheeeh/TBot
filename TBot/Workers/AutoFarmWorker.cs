...
namespace Tbot.Workers {
	public class AutoFarmWorker : WorkerBase {
		...
		private async Task PruneOldReports() {
			...
			foreach (FarmTarget remove in removeReports) {
				FarmTarget updateReport = remove;
		...
		private async Task<Dictionary<int, long>> GetCelestialProbes(List<Celestial> celestials) {
			Dictionary<int, long> celestialProbes = new Dictionary<int, long>();
			foreach (var celestial in celestials) {
				celestialProbes.Add(celestial.ID, celestial.Ships.EspionageProbe);
			}
			return celestialProbes;
		}

		private bool ShouldExclude(Coordinate target) {
			foreach (var exclude in _tbotInstance.InstanceSettings.AutoFarm.Exclude) {
				if ((int) exclude.Galaxy == target.Galaxy && (int) exclude.System == target.System && (int) exclude.Position == target.Position) {
					_tbotInstance.log(LogLevel.Information, LogSender.AutoFarm, $"Skipping target {target.ToString()}: position in exclude list.");
					return true;
				}
			}
			return false;
		...
		private async Task<List<Celestial>> GetScannedTargetsFromGalaxy(int galaxy, int system) {
			GalaxyInfo galaxyInfo = await _ogameService.GetGalaxyInfo(galaxy, system);
			...
			if (_tbotInstance.UserData.fleets.Any(f => f.Mission == Missions.Attack))
		...
		private void AddMoons(List<Celestial> scannedTargets) {
			foreach (Celestial t in scannedTargets.ToList()) {
				Planet planet = t as Planet;
				if (planet == null)
					continue;
				if (planet.Moon != null) {
		...
		private FarmTarget CheckDuplicatesAndGetExisting(Celestial planet) {
			List<FarmTarget> exists = _tbotInstance.UserData.farmTargets.Where(t => t != null && t.Celestial.HasCoords(planet.Coordinate)).ToList();
			if (exists.Count() > 1) {
				// It can exist if was processed in a previous execution
				//Remove all except the first to be able to continue
				FarmTarget firstExisting = exists.First();
		...
		private FarmTarget GetFarmTarget(Celestial planet) {
			...
			FarmTarget target = CheckDuplicatesAndGetExisting(planet);
		...
		private async Task<SpyOriginResult> GetBestOrigin(List<Celestial> closestCelestials,
			...
			foreach (Celestial closest in closestCelestials) {
				// Update ships of the current planet
				Celestial tempCelestial = await _tbotOgameBridge.UpdatePlanet(closest, UpdateTypes.Ships);
				...
				if (freeSlots <= slotsToLeaveFree) {
					List<Fleet> espionageMissions = _calculationService.GetMissionsInProgress(closest.Coordinate, Missions.Spy, _tbotInstance.UserData.fleets);
					if (espionageMissions.Any()) {
						long returningProbes = espionageMissions.Sum(f => f.Ships.EspionageProbe);
					...
					await Task.Delay(RandomizeHelper.CalcRandomInterval(IntervalType.LessThanFiveSeconds), _ct);
					long buildProbes = neededProbes - celestialProbes[closest.ID];
					Resources cost = _calculationService.CalcPrice(Buildables.EspionageProbe, (int) buildProbes);
		...
		private async Task<Dictionary<int, long>> WaitForProbesToComeBack(Celestial origin, Dictionary<int, long> celestialProbes) {

			_tbotInstance.UserData.fleets = await _fleetScheduler.UpdateFleets();
			int interval = _tbotInstance.UserData.fleets.OrderBy(f => f.BackIn)
																.Where(f => f.Mission == Missions.Spy)
																.Where(f => f.Origin.Galaxy == origin.Coordinate.Galaxy && f.Origin.System == origin.Coordinate.System && f.Origin.Position == origin.Coordinate.Position)
																.SingleOrDefault().BackIn ?? 0;
			if (interval > 0) {
				await Task.Delay(interval, _ct);
				Celestial tempCelestial = await _tbotOgameBridge.UpdatePlanet(origin, UpdateTypes.Ships);
				celestialProbes.Remove(origin.ID);
				celestialProbes.Add(origin.ID, tempCelestial.Ships.EspionageProbe);
			}

			return celestialProbes;
		}

		private async Task<List<Celestial>> UpdateSortOrigins(List<Celestial> celestials) {
			List<Celestial> localCelestials = new();
			if (celestials.Where(c => c.Coordinate.Type == Celestials.Moon).Count() > 0 && celestials.Where(c => c.Coordinate.Type == Celestials.Planet).Count() > 0)
				localCelestials = await _tbotOgameBridge.GetEmpire();
			else if (celestials.Where(c => c.Coordinate.Type == Celestials.Moon).Count() > 0)
				localCelestials = await _tbotOgameBridge.GetEmpireMoons();
			else if (celestials.Where(c => c.Coordinate.Type == Celestials.Planet).Count() > 0)
				localCelestials = await _tbotOgameBridge.GetEmpirePlanets();
			else
				localCelestials = await _tbotOgameBridge.GetEmpire();
			return localCelestials.Where(c => celestials.Any(t => t.ID == c.ID)).ToList();
		}
		...
		protected override async Task Execute() {
			...
			try {
				...
				if ((bool) _tbotInstance.InstanceSettings.AutoFarm.Active) {
					...
					List<FarmTarget> WhiteListTarget = await GetData();
					WhiteListTarget.RemoveAll(t => t.Report.Date.AddDays((double) _tbotInstance.InstanceSettings.AutoFarm.WhiteListTarget.MaxDayToKeepWhiteList) < DateTime.Now);
					...
					tempCelestials = await UpdateSortOrigins(tempCelestials);
					...
					try {
						...
						Dictionary<int, long> celestialProbes = await GetCelestialProbes(tempCelestials);
						...
						// Get old report and scan only suitable targets
						_tbotInstance.log(LogLevel.Information, LogSender.AutoFarm, $"There is {WhiteListTarget.Count} in the WhiteList");
						foreach (var range in _tbotInstance.InstanceSettings.AutoFarm.ScanRange) {
							...
							for (int system = startSystem; system <= endSystem; system++) {
								...
								if (stopAutoFarm)
									break;
								if (SettingsService.IsSettingSet(_tbotInstance.InstanceSettings.AutoFarm, "TargetsProbedBeforeAttack") && ((int) _tbotInstance.InstanceSettings.AutoFarm.TargetsProbedBeforeAttack >= 0) && numProbed >= (int) _tbotInstance.InstanceSettings.AutoFarm.TargetsProbedBeforeAttack) {
									_tbotInstance.log(LogLevel.Information, LogSender.AutoFarm, $"Maximum number of targets to probe reached ({_tbotInstance.InstanceSettings.AutoFarm.TargetsProbedBeforeAttack.ToString()}), proceeding to attack.");
									break;
								}

								List<Celestial> scannedTargets = new();
								if (WhiteListTarget.Count >= _tbotInstance.InstanceSettings.AutoFarm.MaxSlots && (bool) (bool) _tbotInstance.InstanceSettings.AutoFarm.WhiteListTarget.Active) {
									WhiteListTarget = WhiteListTarget.OrderBy(t => t.Celestial.Coordinate.Galaxy)
																.ThenBy(t => t.Celestial.Coordinate.System)
																.ThenBy(t => t.Celestial.Coordinate.Position)
																.ToList();
									WhiteListTarget.Where(t => t.Celestial.Coordinate.Galaxy == galaxy).ToList().ForEach(target => scannedTargets.Add(target.Celestial));
									_tbotInstance.log(LogLevel.Information, LogSender.AutoFarm, $"Found {scannedTargets.Count} targets on Galaxy {galaxy}");
									system = endSystem;
								} else {
									...
									if (!scannedTargets.Any())
										continue;
									if (!((bool) _tbotInstance.InstanceSettings.AutoFarm.ExcludeMoons))
								...
								foreach (Celestial planet in scannedTargets) {
									if (stopAutoFarm)
										break;
									// Check if target is below set minimum rank or excluded position.
									if (!IsTargetInMinimumRank(planet, scannedTargets) || ShouldExclude(planet.Coordinate)) {
										continue;
									}
									...
									FarmTarget target = GetFarmTarget(planet);
									if (target == null)
										continue;
									...
									await Task.Delay(RandomizeHelper.CalcRandomInterval(IntervalType.LessThanFiveSeconds)/2, _ct);

									_tbotInstance.UserData.fleets = await _fleetScheduler.UpdateFleets();
									long probesInMission = _tbotInstance.UserData.fleets.Select(c => c.Ships).Sum(c => c.EspionageProbe);
									...
									if (celestialProbes[bestOrigin.Origin.ID] < neededProbes) {
										Celestial tempCelestial = await _tbotOgameBridge.UpdatePlanet(bestOrigin.Origin, UpdateTypes.Ships);
									...
									if (celestialProbes[bestOrigin.Origin.ID] < neededProbes) {
										...
										if (SettingsService.IsSettingSet(_tbotInstance.InstanceSettings.AutoFarm, "BuildProbes") && _tbotInstance.InstanceSettings.AutoFarm.BuildProbes == true) {
											...
											Celestial tempCelestial = await _tbotOgameBridge.UpdatePlanet(bestOrigin.Origin, UpdateTypes.Constructions);
											if (tempCelestial.Constructions.BuildingID == (int) Buildables.Shipyard || tempCelestial.Constructions.BuildingID == (int) Buildables.NaniteFactory) {
												Buildables buildingInProgress = (Buildables) tempCelestial.Constructions.BuildingID;
												_tbotInstance.log(LogLevel.Information, LogSender.AutoFarm, $"Skipping {tempCelestial.ToString()}: {buildingInProgress.ToString()} is upgrading.");
												break;
											...
											long buildProbes = neededProbes - celestialProbes[bestOrigin.Origin.ID];
											Resources cost = _calculationService.CalcPrice(Buildables.EspionageProbe, (int) buildProbes);
											...
											if (tempCelestial.Resources.IsEnoughFor(cost)) {
												...
												_tbotInstance.UserData.fleets = await _fleetScheduler.UpdateFleets();
												List<Fleet> spyMissions = _calculationService.GetMissionsInProgress(bestOrigin.Origin.Coordinate, Missions.Spy, _tbotInstance.UserData.fleets);
												if (spyMissions.Any()) {
													Fleet spyMissionToWait = spyMissions.OrderBy(c => c.BackIn).First();
											...
											celestialProbes = await WaitForProbesToComeBack(bestOrigin.Origin, celestialProbes);
										}
									}
					...
					_tbotInstance.UserData.fleets = await _fleetScheduler.UpdateFleets();
					Fleet lasttReturning = _calculationService.GetLastReturningEspionage(_tbotInstance.UserData.fleets);
					if (lasttReturning != null) {
						int interval = (int) ((1000 * lasttReturning.BackIn) + RandomizeHelper.CalcRandomInterval(IntervalType.AFewSeconds));
					...
					/// Process reports.
					await AutoFarmProcessReports(WhiteListTarget.Count);
					...
					/// Send attacks.
					List<FarmTarget> listFarmTargets = _tbotInstance.UserData.farmTargets.ToList();
					List<FarmTarget> attackTargets;
					if (_tbotInstance.InstanceSettings.AutoFarm.PreferedResource == "Metal")
						attackTargets = listFarmTargets.Where(t => t.State == FarmState.AttackPending).OrderByDescending(t => t.Report.Loot(_tbotInstance.UserData.userInfo.Class).Metal).ToList();
					else if (_tbotInstance.InstanceSettings.AutoFarm.PreferedResource == "Crystal")
						attackTargets = listFarmTargets.Where(t => t.State == FarmState.AttackPending).OrderByDescending(t => t.Report.Loot(_tbotInstance.UserData.userInfo.Class).Crystal).ToList();
					else if (_tbotInstance.InstanceSettings.AutoFarm.PreferedResource == "Deuterium")
						attackTargets = listFarmTargets.Where(t => t.State == FarmState.AttackPending).OrderByDescending(t => t.Report.Loot(_tbotInstance.UserData.userInfo.Class).Deuterium).ToList();
					else
						attackTargets = listFarmTargets.Where(t => t.State == FarmState.AttackPending).OrderByDescending(t => t.Report.Loot(_tbotInstance.UserData.userInfo.Class).TotalResources).ToList();

					if (attackTargets.Count() > 0) {
						Resources resourceAmount = new Resources();
					...
					if (attackTargets.Count() > _tbotInstance.InstanceSettings.AutoFarm.MaxSlots) {
						_tbotInstance.UserData.suitableTargets = attackTargets.ToList();
					}
					...
					foreach (FarmTarget target in attackTargets) {
						...
						Resources loot = target.Report.Loot(_tbotInstance.UserData.userInfo.Class);
						Celestial tempCelestial = tempCelestials.First();
						float cargoBonus = tempCelestial.LFBonuses.GetShipCargoBonus(cargoShip);
						long numCargo = _calculationService.CalcShipNumberForPayload(loot, cargoShip, tempCelestials.First().Researches.HyperspaceTechnology, _tbotInstance.UserData.serverData, cargoBonus, _tbotInstance.UserData.userInfo.Class, _tbotInstance.UserData.serverData.ProbeCargo);
						...
						Ships attackingShips = new Ships().Add(cargoShip, numCargo);
						...
						foreach (Celestial c in closestCelestials) {
							...
							if (tempCelestial.Ships != null && tempCelestial.Ships.GetAmount(cargoShip) >= (numCargo + _tbotInstance.InstanceSettings.AutoFarm.MinCargosToKeep)) {
								...
								if (/*cargoShip == Buildables.EspionageProbe &&*/ SettingsService.IsSettingSet(_tbotInstance.InstanceSettings.AutoFarm, "MinLootFuelRatio") && _tbotInstance.InstanceSettings.AutoFarm.MinLootFuelRatio != 0) {
									...
									var optimalSpeed = _calculationService.CalcOptimalFarmSpeed(tempCelestial.Coordinate, target.Celestial.Coordinate, attackingShips, target.Report.Loot(_tbotInstance.UserData.userInfo.Class), lootFuelRatio, maxFlightTime, tempCelestials.First().Researches, _tbotInstance.UserData.serverData, tempCelestial.LFBonuses, _tbotInstance.UserData.userInfo.Class, _tbotInstance.UserData.allianceClass);
									if (optimalSpeed == 0) {
								...
								FleetPrediction prediction = _calculationService.CalcFleetPrediction(tempCelestial.Coordinate, target.Celestial.Coordinate, attackingShips, Missions.Attack, speed, tempCelestials.First().Researches, _tbotInstance.UserData.serverData, tempCelestial.LFBonuses, _tbotInstance.UserData.userInfo.Class, _tbotInstance.UserData.allianceClass);
							...
							foreach (Celestial closest in closestCelestials) {
								...
								if (
									...
									if (SettingsService.IsSettingSet(_tbotInstance.InstanceSettings.AutoFarm, "BuildCargos") && _tbotInstance.InstanceSettings.AutoFarm.BuildCargos == true) {
										long neededCargos = numCargo + (long) _tbotInstance.InstanceSettings.AutoFarm.MinCargosToKeep - tempCelestial.Ships.GetAmount(cargoShip);
										Resources cost = _calculationService.CalcPrice(cargoShip, (int) neededCargos);
										if (tempCelestial.Resources.IsEnoughFor(cost)) {
											...
											long buildableCargos = _calculationService.CalcMaxBuildableNumber(cargoShip, tempCelestial.Resources);
						...
						if (_tbotInstance.UserData.slots.Free > slotsToLeaveFree && slotUsed.Count() < MaxSlots) {
							...
							int fleetId = await _fleetScheduler.SendFleet(fromCelestial, attackingShips, target.Celestial.Coordinate, Missions.Attack, speed);

							if (fleetId > (int) SendFleetCode.GenericError) {
								freeSlots--;
								tempCelestials.Where(c => c.ID == fromCelestial.ID).First().Ships.Remove(cargoShip, (int) numCargo);
								tempCelestials.Where(c => c.ID == fromCelestial.ID).First().Resources.Deuterium -= _calculationService.CalcFleetPrediction(fromCelestial.Coordinate, target.Celestial.Coordinate, attackingShips, Missions.Attack, speed, tempCelestials.First().Researches, _tbotInstance.UserData.serverData, fromCelestial.LFBonuses, _tbotInstance.UserData.userInfo.Class, _tbotInstance.UserData.allianceClass).Fuel;
							} else if (fleetId == (int) SendFleetCode.AfterSleepTime) {
								stop = true;
								return;
							}
				...
				if (!_tbotInstance.UserData.isSleeping) {
					if (stop) {
						...
						long interval;
						try {
							interval = (int) ((1000 * orderedFleets.Last().BackIn) + RandomizeHelper.CalcRandomInterval(IntervalType.SomeSeconds));
						} catch {
		...
		/// </summary>
		private async Task AutoFarmProcessReports(int whiteListTargetsCount) {
			...
			List<FarmTarget> tempReport = new();
			foreach (EspionageReportSummary summary in summaryReports) {
				...
				try {
					...
					if (_tbotInstance.UserData.farmTargets.Any(t => t.HasCoords(report.Coordinate)) || (_tbotInstance.InstanceSettings.AutoFarm.ReadAllReportsEvenThoseNotScannedByTBot && !_tbotInstance.UserData.celestials.Any(c => c.HasCoords(report.Coordinate)))) {
						FarmTarget target;
						...
						if (matchingTarget.Count() == 0) {
							...
							GalaxyInfo galaxyInfo = await _ogameService.GetGalaxyInfo(report.Coordinate.Galaxy, report.Coordinate.System);
							Planet planet = galaxyInfo.Planets.FirstOrDefault(p => p != null && p.Inactive && !p.Administrator && !p.Banned && !p.Vacation && p.HasCoords(report.Coordinate));
							if (planet != null) {
						...
						FarmTarget newFarmTarget = target;
			...
			if (whiteListTargetsCount <= 0 && tempReport.Count > 0 && (bool) _tbotInstance.InstanceSettings.AutoFarm.WhiteListTarget.Active)
				await ProcessReportToWhiteList(tempReport);

			await _ogameService.DeleteAllEspionageReports();

		}
		private async Task ProcessReportToWhiteList(List<FarmTarget> targets) {
			List<FarmTarget> whiteListReports = new();
			foreach (FarmTarget target in targets) {
				EspionageReport report = target.Report;
				if (report.Loot(_tbotInstance.UserData.userInfo.Class).TotalResources < _tbotInstance.InstanceSettings.AutoFarm.WhiteListTarget.MinResources)
					continue;
				if ((report.HasDefensesInformation && report.DefenceValue() >= _tbotInstance.InstanceSettings.AutoFarm.WhiteListTarget.MaxDefenceValue) || !report.HasDefensesInformation)
					continue;
				if ((report.HasFleetInformation && report.FleetValue() >= _tbotInstance.InstanceSettings.AutoFarm.WhiteListTarget.MaxFleetValue) || !report.HasFleetInformation)
					continue;
				whiteListReports.Add(target);
			}
			await SetData(whiteListReports);
		}
		private async Task<List<FarmTarget>> GetData() {
			List<FarmTarget> result;
			try {
				result = await _tbotInstance.AnyData(Feature.AutoFarm, "autofarmWhiteList", _tbotInstance.UserData.serverData.Name +"_"+ _tbotInstance.UserData.userInfo.PlayerID) ?
								JsonConvert.DeserializeObject<List<FarmTarget>>(await _tbotInstance.ReadData(Feature.AutoFarm, "autofarmWhiteList", _tbotInstance.UserData.serverData.Name +"_"+ _tbotInstance.UserData.userInfo.PlayerID)) :
								new();
			} catch (Exception ex) {
				DoLog(LogLevel.Warning, $"Cannot read autofarmWhiteList: {ex.GetType().Name}: {ex.Message}");
				result = new();
			}
			return result;
		}
		private async Task SetData(List<FarmTarget> data) {
			try {
				await _tbotInstance.WriteData(Feature.AutoFarm, "autofarmWhiteList", data, _tbotInstance.UserData.serverData.Name +"_"+ _tbotInstance.UserData.userInfo.PlayerID);
			} catch (Exception ex) {
				DoLog(LogLevel.Error, $"autofarmWhiteList save failed: {ex.GetType().Name}: {ex.Message}");
			}
		}
	}
}